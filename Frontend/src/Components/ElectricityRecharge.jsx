...
export default function ElectricityRecharge() {
  ...
  const [rechargeUser, setRechargeUser] = useState({ username: "500032", pwd: "k0ly9gts" });
 ...
 const operators = [
  ...
  { code: "BMESTU", name: "Brihan Mumbai Electric Supply and Transport Undertaking" },
  { code: "APCPDCL", name: "Central Power Distribution Company of Andhra Pradesh Ltd" },
  { code: "CESCOM", name: "Chamundeshwari Electricity Supply Corporation Ltd. (Cesc,Mysore)" },
  { code: "CSPDCL", name: "Chhattisgarh State Power Distribution Company Ltd. (CSPDCL)" },
  ...
  { code: "MPPKVVCLMR", name: "Madhya Pradesh Madhya Kshetra Vidyut Vitaran-RURAL" },
  { code: "MPPKVVCL", name: "Madhya Pradesh Poorv Kshetra Vidyut Vitaran-URBAN" },
  ...
  useEffect(() => {
    const detectOperator = async () => {
      if (formData.consumerNumber.length === 10) {
        setDetecting(true);
        try {
          const res = await fetch(`${API_URL}/api/lookup?number=${formData.consumerNumber}`);
...
console.log("Sending recharge request:", payload);
  ...
  return (
    <div style={styles.container}>
      <Nav />
      <Hero title="Instant Electricity Recharge" subtitle="Fast, secure, and reliable electricity recharges for all operators" />
            ...
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                ...
                <div>
                  ...
                  <p style={styles.cardSubtitle}>Pay your bills for all providers</p>
                </div>
              </div>
                ...
                <form onSubmit={handleRecharge}>
                  <div style={styles.formGroup}>
                    ...
                    <input
                      ...
                      style={styles.input}
                    />
                    {detecting && <div style={styles.detectingText}>Detecting operator...</div>}
                  </div>
                  ...
                  <button type="submit" disabled={loading} style={{ ...styles.rechargeBtn, ...(loading ? styles.btnDisabled : {}) }}>
                    {loading ? <div style={styles.loadingSpinner}></div> : <> <Zap size={20} /> Pay Bill </>}
                  </button>
              ...
              <div style={styles.cardBody}>
                {transactions.length === 0 ? (
                  <div style={styles.emptyState}>
                    ...
                    <p style={styles.emptySubtext}>Your electricity payment history will appear here</p>
                  </div>
                ) : (
