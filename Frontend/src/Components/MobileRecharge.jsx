                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    if (value.length === 10) {
                      try {
                        const res = await fetch(
                          `https://www.nixinfo.in/api/operator-and-circle-api?status=status&emailid=vikashpandit380@gmail.com&ctrlkey=e510c57d498ae3302f5c61ba2ebff78ef08fd774774096683&mobnumber=${value}`
                        );
                        ...
                        if (data.Operator && data.Circle) {
                          ...
                          const operatorMatch = operators.find((op) =>
                            data.Operator.toLowerCase().includes(
                              op.name.toLowerCase()
                            )
                          );
                          const circleMatch = circles.find((c) =>
                            data.Circle.toLowerCase().includes(
                              c.name.toLowerCase()
                            )
                          );
